...
function AdminDashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  function publicHouseLink(house: Pick<House, 'slug'>) {
    return `${publicSiteUrl}/#/links/${house.slug}`;
  }

  function publicHouseOpenLink(house: Pick<House, 'slug'>) {
    const cacheVersion = `${house.slug}-${Date.now()}`;
    return `${publicSiteUrl}/?v=${encodeURIComponent(cacheVersion)}#/links/${house.slug}`;
  }
